...
async function _messageHandler(event) {
...
tv.getPerformance = async (testResults, isIgnoreError=false) => {
  ...
  if (!isProcessError && !isProcessEnd && testResults.performanceSummary.length && !testResults.isDeepTest) {
    const lastRes = testResults.performanceSummary[testResults.performanceSummary.length - 1] // (!) Previous value maybe in testResults.filteredSummary
